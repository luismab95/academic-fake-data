...
app.get("/academic/record", async (req, res) => {
  ...
  try {
    const queryUniversity = await pool.query(
      "SELECT * FROM universidad ORDER BY RANDOM() LIMIT FLOOR(RANDOM() * 3) + 1;",
      []
    );

    const randomUser = await pool.query(
      "SELECT * FROM estudiantes ORDER BY RANDOM() LIMIT $1",
      [queryUniversity.rows.length]
    );

    const response = [];

    for (let [index, user] of randomUser.rows.entries()) {
      ...
      const data = {
        university: queryUniversity.rows[index],
...
app.get("/academic/record/detail", async (req, res) => {
  const { studentId, universityId } = req.query;

  try {
    const queryUniversity = await pool.query(
      "SELECT * FROM UNIVERSIDAD WHERE id_universidad = $1",
      [Number(universityId)]
    );
